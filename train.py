...
def photos(args):
    ...
    for i in ['.gitkeep', '.DS_Store', 'trash']:
        if i in photo_nums:
            photo_nums.remove(i)
    ...
    if args.cleanup:
        removed = []
        for i in photo_nums:
            if os.path.getsize(data_folder + str(i) + '.jpg') in [874, 1854, 2588, 5106, 11197, 12814, 6305]:  # [4019, 16161, 35, 1485, 7742, 34262, 5098] 40000枚中これらのみ発見
                photo_nums.remove(i)
                removed.append(i)
                shutil.move(data_folder + str(i) + '.jpg', data_folder + 'trash/' + str(i) + '.jpg')
        with open('removed.txt', 'w') as f:
            pickle.dump(removed, f)
    if args.total_photo_num != -1:
...
def main():
    ...
    train = chainer.datasets.TransformDataset(train, Transform(args, True))
    test = chainer.datasets.TransformDataset(test, Transform(args, False))
